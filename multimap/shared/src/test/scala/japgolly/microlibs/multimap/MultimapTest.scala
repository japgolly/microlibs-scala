...
object MultimapTest extends TestSuite {
  ...
  //   def comm[R: Eq](name: => String, z: MM, r: MM => R, f: MM => MM, g: MM => MM) =
  ...
  //                                      (implicit c: Option[Commutative[L]], E: Eq[L[A]]): Gen[PropInputs[L, A]] = {
  //   val gla = gl(ga)
  //   for {
  ...
  // val genArraySeq: Gen[PropInputs[ArraySeq, Long]] = gen(Gen.long, _.arraySeq)

  override def tests = Tests {
  //   "list"     - genList    .mustSatisfyE(_.eval)
  //   "set"      - genSet     .mustSatisfyE(_.eval)
  //   "vector"   - genVector  .mustSatisfyE(_.eval)
  //   "arraySeq" - genArraySeq.mustSatisfyE(_.eval)
    // TODO: Port MultimapTest to ScalaCheck
  }
}
